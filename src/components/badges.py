...
def patient_chip(name: str) -> None:
    ...
    st.markdown(
        f"<span title='{safe_name}' style='background:#eef2ff;border:1px solid #c7d2fe;border-radius:999px;padding:0.35rem 0.55rem;margin:0.15rem;display:inline-block'><b>{initials}</b> {safe_name}</span>",
        unsafe_allow_html=True,
    )
